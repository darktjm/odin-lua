...
void
ExecInternal(tp_FilHdr FilHdr, tp_Status DepStatus, tp_Date DepModDate)
{
   tps_ExecSpc _ExecSpc;
   tp_ExecSpc ExecSpc = &_ExecSpc;
   tp_Job Job;
   tp_Tool Tool;
   tp_TClass TClass;
   ...
   case TC_Apply:{
         ...
         Set_LocElm(FilHdr,
                    Make_ApplyLocElm(Inputs[0], FilHdr, InFileName));
         Pop_ContextFilHdr();
         break;
      }
   case TC_DerefPrmVal:{
   ...
   case TC_PntrElm:{
         ...
         LocElm = 0;
         ElmFilHdr =
             Do_Key(Copy_FilHdr(Inputs[0]), FilHdr_Key(StrBuf, FilHdr));
         if (ElmFilHdr != ERROR) {
            LocElm = Make_LocElm(ElmFilHdr, RootFilPrm, FilHdr);
            Ret_FilHdr(ElmFilHdr);
         }
   ...
   case TC_VirTargets:{
         ...
         if (InFD != NIL) {
            ...
            LocElm =
                Make_TargetsLocElm(FilHdr, InFD, InFileName, DepModDate,
   ...
   case TC_CopyCmd:{
         DestFilHdr = Deref(LocHdr_FilHdr
                            (FilPVal_LocHdr
                             (FilPrm_FilPVal(FilHdr_FilPrm(FilHdr)))));
         {
            if (DestFilHdr == ERROR) {
   ...
   default:{
         FATALERROR("illegal system tool");
      }
   }
