...
static boolean Is_CopyDone(tp_FilHdr FilHdr)
{
   ...
   Ret_FilHdr(OrigFilHdr);
   Ret_FilHdr(DestFilHdr);
   return OK;
}

static void GetFile(tp_FilHdr FilHdr)
{
   ...
   if (IsSource(FilHdr)) {
      ...
      if (!IsTgtValUpToDate(FilHdr)) {
         ...
         Set_TgtValStatus(FilHdr, Status);
      }
      Set_PndFlag(FilHdr, FALSE);
      Set_TgtValPndFlag(FilHdr, FALSE);
      return;
   ...
   for (FilInp = LocInp_FilInp(FilHdr_LocInp(FilHdr));
      ...
      if (NeedsData(InpFilHdr, InpKind)) {
         ...
         if (MaxSonModDate < ModDate)
            MaxSonModDate = ModDate;
   ...
   if (MinStatus == STAT_Unknown) {
      Do_Log("Pending circular computation for", FilHdr,
             LOGLEVEL_Circular);
      Set_ListStatus(FilHdr, STAT_Unknown);
      goto done;
   }
   if (IsUpToDate(FilHdr)) {
      FORBIDDEN(!(IsStructMem(FilHdr) || IsKeyListElm(FilHdr)
                  || FilHdr_Status(FilHdr) == STAT_Circular));
      if (Is_PRB_Status(MinStatus)) {
         Set_ListStatus(FilHdr, STAT_Pending);
         Broadcast_Mod(FilHdr, MODKIND_Input, STAT_Pending);
      }
      goto done;
   }
   ...
   if ((FilHdr_ModDate(FilHdr) > 0
      ...
      if (MinStatus > FilHdr_MinErrStatus(FilHdr)) {
         MinStatus = FilHdr_MinErrStatus(FilHdr);
      }
      if (MinStatus == STAT_Error && IsTargets(FilHdr)) {
         MinStatus = STAT_TgtValError;
      }
...
static void ClearAll_SCC(tp_FilHdr FilHdr, boolean DataFlag)
{
   tp_FilElm FilElm;
   ...
   if (FilHdr_ElmDepth(FilHdr) != ElmDepth) {
      FORBIDDEN(FilHdr_ElmDepth(FilHdr) != 0
                && FilHdr_ElmDepth(FilHdr) > ElmDepth);
      return;
   }
...
static void
GetReqs(tp_FilHdr FilHdr, boolean NameFlag, boolean DataFlag, int *ETPtr)
{
   ...
   *ElmTagPtr += 1;
   ...
   if (DataFlag) {
      ...
      MaxModDate = 1;
      Set_ElmModDate(FilHdr, MaxModDate);
   }
      ...
      if (DataFlag || ListFlag) {
         ...
         if (FilHdr_ElmTag(ElmFilHdr) != 0) {
            ...
            if (ElmTag > FilHdr_ElmTag(SCC_ElmFilHdr)) {
               ...
               ElmTag = FilHdr_ElmTag(SCC_ElmFilHdr);
            }
            Ret_FilHdr(SCC_ElmFilHdr);
         }
         ...
         if (DataFlag) {
            if (MinStatus > Status)
               MinStatus = Status;
            if (MaxModDate < ModDate)
               MaxModDate = ModDate;
         }

         if (ListFlag) {
            if (MinNameStatus > Status)
               MinNameStatus = Status;
            if (MaxNameModDate < ModDate)
               MaxNameModDate = ModDate;
   ...
   if (FilHdr == SCC_FilHdr) {
      ClearAll_SCC(FilHdr, DataFlag);
   }
   Ret_FilHdr(SCC_FilHdr);

 done:;
   if (DepthSet) {
      Set_AnyOKDepth(FilHdr, 0);
   }
}

void GetAllReqs(tp_FilHdr FilHdr, tp_InpKind InpKind)
{
   boolean DataFlag, NameFlag;
