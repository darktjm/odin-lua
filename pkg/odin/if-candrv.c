...
tp_DrvPth Get_DrvPth(tp_FilHdr FilHdr, tp_FilTyp ToFilTyp)
{
   tp_DrvPth DrvPth, GenericDrvPth;
   boolean IsGeneric;
   tp_FilHdr GenericFilHdr, TmpGenericFilHdr;

   if (FilHdr == ERROR || ToFilTyp == ERROR)
      return ERROR;
   ...
   if (IsGeneric) {
      ...
      while (IsGeneric) {
         ...
         GenericFilHdr =
             Do_DrvPth(GenericFilHdr, RootFilPrm, RootFilPrm,
                       GenericDrvPth);
         IsGeneric = FALSE;
         {
            if (GenericFilHdr == TmpGenericFilHdr) {
               ...
               AppendDrvPth(&DrvPth, GenericDrvPth);
            }
         }
      }
...
tp_PrmTypLst DrvPth_PrmTypLst(tp_DrvPth DrvPth)
{
   tp_DrvEdg DrvEdg;

   DrvEdg = DrvPth_DrvEdg(DrvPth);
   if (DrvEdg != NIL) {
      return DrvEdg_PrmTypLst(DrvEdg);
   }
   if (DrvPth_FKind(DrvPth) == FK_PntrHo) {
      return FilTyp_MapPrmTypLst(DrvPth_FilTyp(DrvPth));
   }
   return (tp_PrmTypLst) NIL;
}

tp_DrvPth Find_GroupingDrvPthElm(tp_DrvPth DrvPth)
{
...
tp_FilHdr
Do_DrvPth(tp_FilHdr FilHdr,
          tp_FilPrm InhFilPrm, tp_FilPrm PrecFilPrm, tp_DrvPth DrvPth)
{
   tp_DrvPth DrvPthElm, GroupingDrvPthElm;
   tp_FilTyp FilTyp;
   tp_FKind FKind;
...
tp_FilHdr
Do_Deriv(tp_FilHdr FilHdr,
         tp_FilPrm InhFilPrm, tp_FilPrm PrecFilPrm, tp_FilTyp ToFilTyp)
{
   tp_DrvPth DrvPth;
   ...
   if (FilHdr_FilTyp(FilHdr) == ToFilTyp) {
      if (PrecFilPrm != RootFilPrm) {
         Do_Log("Ignoring parameters of", FilHdr, LOGLEVEL_IgnorePrm);
      }
      return FilHdr;
   }
...
tp_FilHdr Do_Key(tp_FilHdr FilHdr, tp_Key Key)
{
   ...
   if (IsSource(FilHdr)) {
      if (strcmp(Key, "..") == 0) {
         ...
         if (DirSymFilHdr == SymDirFilHdr) {
            Ret_FilHdr(DirSymFilHdr);
            Ret_FilHdr(SymDirFilHdr);
            return DirFilHdr;
         }
         Ret_FilHdr(DirFilHdr);
         Ret_FilHdr(SymDirFilHdr);
         return FilHdr_AliasFilHdr(DirSymFilHdr);
      }
      ...
      if (FilHdr == CacheDirFilHdr) {
         ...
         Ret_FilHdr(FilHdr);
         return CacheFileName_FilHdr(Key);
      }

      return Get_KeyDrv(FilHdr, FK_SrcReg, Key);
   }

   if (IsVirDir(FilHdr)) {
      return Get_KeyDrv(FilHdr, FK_VirDirElm, Key);
   }

   if (IsTargets(FilHdr)) {
      return Get_KeyDrv(FilHdr, FK_ActTgtText, Key);
   }
   ...
   if (IsDrvDirElm(FilHdr)) {
      return Get_KeyDrv(FilHdr, FK_DrvDirElm, Key);
   }
...
tp_FilHdr Str_FilHdr(tp_Str Str, tp_PrmTyp PrmTyp)
{
   if (strcmp(Str, "") == 0) {
...
tp_FilHdr Do_VTgt(tp_FilHdr FilHdr, tp_Key Key)
{
   if (FilHdr_FilTyp(FilHdr) == TargetsFilTyp) {
      return Get_KeyDrv(FilHdr, FK_VirTgtText, Key);
   }
...
void WriteDrvHelp(tp_FilDsc FilDsc, tp_FilHdr FilHdr)
{
...
void WritePrmHelp(tp_FilDsc FilDsc, tp_FilHdr FilHdr, tp_FilPrm FilPrm)
{
   tp_FilHdr ValFilHdr;
   tp_FilTyp FilTyp;
...
void WriteNameDesc(tp_FilDsc FilDsc, tp_Str Name, tp_Desc Desc)
{
   tps_Str Msg;
...
tp_FilHdr Get_BaseVTgtFilHdr(tp_FilHdr FilHdr)
{
   FORBIDDEN(IsTargetsPtr(FilHdr));
   while (!IsSource(FilHdr)) {
      if (IsVTgtText(FilHdr) || FilHdr_FilTyp(FilHdr) == VirTargetsFilTyp) {
         return FilHdr_Father(FilHdr_Father(FilHdr));
      }
      if (IsVTgt(FilHdr)) {
         return FilHdr_Father(FilHdr_Father(FilHdr_Father(FilHdr)));
      }
...
static tp_LocHdr PrmValNod_LocHdr(tp_Nod Nod, tp_PrmTyp PrmTyp)
{
   tp_PrmFHdr PrmFHdr;
   tp_FilHdr FilHdr;
   tp_FilPrm FilPrm;
   ...
   default:{
         FATALERROR("illegal parameter value node");
      }
   }
   if (FilHdr == ERROR) {
      return ERROR;
...
static tp_FilPrm Append_PrmNod(tp_FilPrm FilPrm, tp_Nod PrmNod)
{
   tp_PrmTyp PrmTyp;
   ...
   for (PrmValNod = Nod_Son(1, PrmValsNod);
        PrmValNod != NIL; PrmValNod = Nod_Brother(PrmValNod)) {
      LocHdr = PrmValNod_LocHdr(PrmValNod, PrmTyp);
      if (LocHdr == NIL) {
         return ERROR;
      }
      FilPVal = Add_PValInf(FilPVal, LocHdr, (tp_LocPVal) NIL);
   }
   LocHdr = NIL;
...
Apply_OprNods(tp_FilHdr FilHdr, tp_FilPrm FilPrm, tp_Nod OprNod)
{
   ...
   while (OprNod != NIL) {
      ...
      OprNod = Nod_Brother(OprNod);
   }

   return New_PrmFHdr(FilHdr, FilPrm);
}

tp_PrmFHdr Nod_PrmFHdr(tp_Nod Nod)
{
   tp_FilHdr FilHdr;
   ...
   case WORD:{
         ...
         return New_PrmFHdr(FilHdr, RootFilPrm);
      }
   case DRVFIL:{
         break;
      }
   ...
   case STRING:{
         FilHdr =
             Str_FilHdr(Sym_Str(Nod_Sym(Nod_Son(1, RootNod))), NullPrmTyp);
         break;
      }
   default:{
         FATALERROR("Unexpected node type.\n");
...
tp_LocElm
Make_ApplyLocElm(tp_FilHdr InFilHdr,
                 tp_FilHdr FilHdr, tp_FileName OprFileName)
{
   ...
   for (Str = ReadLine(StrSegBuf, FilDsc);
        Str != NIL; Str = ReadLine(StrSegBuf, FilDsc)) {
      (void) strcat(Str, "\n");
      StrLen += strlen(Str);
      if (StrLen >= MAX_Str) {
        ...
        tp_FilPrm FilPrm, tp_FilTyp FilTyp, tp_FilHdr ListFilHdr)
{
   ...
   if (!IsRef(FilHdr)) {
      ElmFilHdr =
          Do_Deriv(Copy_FilHdr(FilHdr), RootFilPrm, FilPrm, FilTyp);
      {
         if (ElmFilHdr != ERROR) {
...
tp_LocElm Make_MapLocElm(tp_FilHdr FilHdr, tp_FilHdr ListFilHdr)
{
...
void Local_Get_OdinFile(tp_Str OdinExpr, boolean NeedsData)
{
   tp_Nod Root;
   tp_PrmFHdr PrmFHdr;
   tp_FilHdr FilHdr;
