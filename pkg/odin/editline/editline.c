...
STATIC STATUS do_hist(MOVE move)
{
   CHAR *p;
   int i;
...
STATIC CHAR *search_hist(CHAR * search, MOVE move)
{
...
STATIC STATUS TTYspecial(UINT c)
{
   if (ISMETA(c))
      return CSdispatch;
...
STATIC CHAR *find_word(void)
{
   ...
   for (p = &Line[Point]; p > Line && strchr(SEPS, (char) p[-1]) == NULL;
        p--)
      continue;
   len = Point - (p - Line) + 1;
