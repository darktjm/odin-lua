...
boolean Empty_DummyStack(void)
{
   return (num_DummyStack == 0);
}

void Push_DummyStack(tp_Nod Node, int Offset)
{
   if (Node == ERROR)
      FATALERROR("illegal arg to Push_DummyStack");
   if (num_DummyStack == MAX_DummyStack)
      FATALERROR("DummyStack overflow");
   DummyStack[num_DummyStack].Node = Node;
   DummyStack[num_DummyStack].Offset = Offset;
   num_DummyStack += 1;
}

void Pop_DummyStack(tp_Nod * NodePtr, int *OffsetPtr)
{
   if (num_DummyStack == 0)
      FATALERROR("DummyStack underflow");
   num_DummyStack -= 1;
