...
void Gen_Grammar(tp_FilDsc FilDsc, tp_Nod Includes_Nod, tp_Nod Scanner_Nod,
                 tp_Nod Rules_Nod)
{
   ...
   for (i = 1; i <= Nod_NumSons(Rules_Nod); i++) {
      ...
      for (i1 = 1; i1 < Nod_NumSons(AltList_Nod); i1++) {
         ...
         NumSons = 0;
         Gen_Seq(FilDsc, Nod_Son(1, Alternative_Nod), Symbol, TRUE,
                 &Offset, &NumSons);
...
void Gen_LHS(tp_FilDsc FilDsc, tp_Sym Symbol, int Offset)
{
   Write(FilDsc, "\n x");
   Write(FilDsc, Sym_Str(Symbol));
   if (Offset > 0) {
      Write(FilDsc, "_");
      WriteInt(FilDsc, Offset);
   }
   Writeln(FilDsc, " :");
}

void Gen_Rule(tp_FilDsc FilDsc, tp_Nod Node, tp_Sym Symbol,
              int *Offset_Ptr, int *NumSons_Ptr)
{
   ...
   default:{
         fatal_err("illegal node type in Gen");
      }
   }
}

void Gen_Seq(tp_FilDsc FilDsc, tp_Nod Node, tp_Sym Symbol, boolean ensymf,
             int *Offset_Ptr, int *NumSons_Ptr)
{
   int i;
   ...
   for (i = 1; i <= Nod_NumSons(Node); i++) {
      Gen_Token(FilDsc, Nod_Son(i, Node), Symbol, ensymf, Offset_Ptr,
                NumSons_Ptr);
   }
}

void Gen_Token(tp_FilDsc FilDsc, tp_Nod Node, tp_Sym Symbol,
               boolean ensymf, int *Offset_Ptr, int *NumSons_Ptr)
{
   tp_Str Str;
   tp_Sym Sym;
   ...
   case DSTRNG:{
         ...
         if (Get_Att(ATP_Lexical, Sym).Int != LX_NonLeaf) {
            Increment_Errors();
            SystemError
                ("\"%s\" used but not defined as a non-leaf token.\n",
                 Sym_Str(Sym));
         }
   ...
   case LIST:{
         *Offset_Ptr += 1;
         if (*Offset_Ptr % MAX_Dummys == 0) {
            ...
            fatal_err("Dummy Overflow");
         }
         Gen_DummySym(FilDsc, Symbol, *Offset_Ptr);
         *NumSons_Ptr += 1;
         if (ensymf)
            Push_DummyStack(Node, *Offset_Ptr);
         break;
      }
   default:{
         fatal_err("illegal node type in Gen_Token");
      }
   }
}

void Gen_DummySym(tp_FilDsc FilDsc, tp_Sym Symbol, int Offset)
{
   Write(FilDsc, "x");
   Write(FilDsc, Sym_Str(Symbol));
   if (Offset > 0) {
...
void Gen_Act(tp_FilDsc FilDsc, tp_Nod Alternative_Nod, int NumSons)
{
   tp_Nod NodeName_Nod;
   int NodeType;
   tp_Str Str;
...
void Write_Act(tp_FilDsc FilDsc, int NodeType, int NumSons)
{
