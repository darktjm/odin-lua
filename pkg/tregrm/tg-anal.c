...
void Analyze(tp_Nod Scanner_Nod, tp_Nod Nodes_Nod, tp_Nod Rules_Nod)
{
   ...
   for (i = 1; i <= Nod_NumSons(Scanner_Nod); i++) {
      ...
      if (Is_UserLeafDef(LexDef_Nod)) {
         ...
         Attrib.Sym = Sym;
         Set_Att(ATP_LexType, Lex_Sym, Attrib);
      }
   }
   ...
   for (i = 1; i <= Nod_NumSons(Nodes_Nod); i++) {
      ...
      Attrib.Int = i;
      Set_Att(ATP_Nodes, Nod_Sym(NodeType_Nod), Attrib);
   }
   ...
   for (i = 1; i <= Nod_NumSons(Rules_Nod); i++) {
      ...
      Attrib.Int = i;
      Set_Att(ATP_NonTerminal, Nod_Sym(Name_Nod), Attrib);
   }
}
