...
static tp_FilTyp Init_SystemFilTyp(tp_FTName FTName)
{
   tp_FilTyp FilTyp;
...
void Init_FilTyps(void)
{
   ...
   ExpandHooksFilTyp =
       Lookup_FilTyp(Sym_Str(Str_Sym("odin*expand_hooks")));
...
void Set_Tool(tp_FilTyp FilTyp, tp_Tool Tool)
{
   FORBIDDEN(FilTyp == ERROR);
   FilTyp->Tool = Tool;
}

tp_Desc FilTyp_Desc(tp_FilTyp FilTyp)
{
   if (FilTyp == ERROR)
      return ERROR;
   return FilTyp->Desc;
...
void Set_FilTyp_Desc(tp_FilTyp FilTyp, tp_Desc Desc, boolean Hidden)
{
...
static tp_FilTyp New_FilTyp(tp_FTName FTName)
{
   tp_FilTyp FilTyp;
...
static tp_FilTyp New_SecOrdFilTyp(tp_FTName FTName, tp_FilTyp ArgFilTyp)
{
   tp_FilTyp FilTyp;
   tp_Tool Tool;
   tp_InpSpc InpSpc;
   ...
   Tool->Package = Sym_Str(Str_Sym("odin"));
   {
      if (strcmp(FTName, "lookup") == 0) {
         ...
         SystemError
             ("Must be lookup, map, recurse, extract, or delete: %s");
         return ObjectFilTyp;
      }
   }
...
tp_FilTyp Create_StructFilTyp(tp_Package Package, tp_FTName FTName)
{
   tp_FilTyp FilTyp;
   tps_Str StrBuf;
...
tp_FilTyp Create_OutputFilTyp(tp_Package Package, tp_Package ToolPackage,
                              tp_FTName FTName)
{
   tps_Str StrBuf;
   tp_FilTyp FilTyp;
...
static tp_FilTyp FTName_FilTyp(tp_FTName FTName)
{
   tp_FilTyp FilTyp;
...
static tp_FilTyp FTName_SecOrdFilTyp(tp_FTName FTName, tp_FilTyp ArgFilTyp)
{
   tp_FilTyp FilTyp;
...
tp_FilTyp Lookup_FilTyp(tp_FTName FTName)
{
   tp_FilTyp FilTyp;
...
tp_FilTyp Lookup_SecOrdFilTyp(tp_FTName FTName, tp_FilTyp ArgFilTyp)
{
   tp_FilTyp FilTyp;
...
static boolean Is_LaterSrcTyp(Pattern1, IsPrefix1, Pattern2, IsPrefix2)
tp_Pattern Pattern1;
boolean IsPrefix1;
tp_Pattern Pattern2;
boolean IsPrefix2;
{
   ...
   if (strlen(Pattern1) != strlen(Pattern2)) {
      return ((int) strlen(Pattern1) < (int) strlen(Pattern2));
   }
   return (strcmp(Pattern1, Pattern2) > 0);
}

tp_SrcTyp Lookup_SrcTyp(tp_Pattern Pattern, boolean IsPrefix)
{
...
void Set_SrcTyp_FilTyp(tp_SrcTyp SrcTyp, tp_FilTyp FilTyp)
{
   FORBIDDEN(SrcTyp == ERROR || FilTyp == ERROR);
   SrcTyp->FilTyp = FilTyp;
}

tp_FilTyp SrcTyp_FilTyp(tp_SrcTyp SrcTyp)
{
   if (SrcTyp == ERROR)
      return ERROR;
...
void Print_Info(tp_FilDsc FilDsc)
{
   tp_FilTyp FilTyp;
   tp_DrvEdg DrvEdg;
   tp_EqvEdg EqvEdg;
   ...
   for (FilTyp = FilTypS; FilTyp != NIL; FilTyp = FilTyp->Link) {
      ...
      for (DrvEdg = FilTyp->DrvEdg;
           DrvEdg != 0; DrvEdg = DrvEdg_Next(DrvEdg)) {
   ...
   for (FilTyp = FilTypS; FilTyp != NIL; FilTyp = FilTyp->Link) {
      if (FilTyp->EqvEdg != 0) {
         ...
         for (EqvEdg = FilTyp->EqvEdg;
              EqvEdg != 0; EqvEdg = EqvEdg_Next(EqvEdg)) {
            Write(FilDsc, " == ");
            Print_FilTyp(FilDsc, EqvEdg_FilTyp(EqvEdg));
         }
         Writeln(FilDsc, "");
      }
   }
   ...
   for (FilTyp = FilTypS; FilTyp != NIL; FilTyp = FilTyp->Link) {
      for (CastEdg = FilTyp->CastEdg;
           CastEdg != 0; CastEdg = CastEdg_Next(CastEdg)) {
         Write(FilDsc, "   ");
         Print_FilTyp(FilDsc, FilTyp);
         Write(FilDsc, " -> ");
...
void Print_FilTyp(tp_FilDsc FilDsc, tp_FilTyp FilTyp)
{
   Write(FilDsc, FilTyp->FTName);
   if (FilTyp->ArgFilTyp != NIL) {
      Write(FilDsc, "=:");
      Write(FilDsc, FilTyp->ArgFilTyp->FTName);
   }
   if (FilTyp->MemEdg != 0)
      Print_MemEdgs(FilDsc, FilTyp->MemEdg);
}

void Write_FilTyps(FILE * DRVGRF_FILE, FILE * DG_C_FILE)
{
   tp_SrcTyp SrcTyp;
   int iFilTyp;
   ...
   (void) fprintf(DRVGRF_FILE,
                  "%d .%s\1 .%s\1 %d %d %d %d %d %d %d %d %d %d %d\n",
                  FilTyp->FTClass, FilTyp->FTName, FilTyp->Desc,
                  FilTyp->HelpLevel, iArgFilTyp, iTool, iMemEdg, iEqvEdg,
                  iCastEdg, iDrvEdg, iMapPrmTypLst, FilTyp->IsCopy,
