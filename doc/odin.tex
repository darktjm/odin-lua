\documentstyle[url,hyperref]{report}
\topmargin -0.25in
\textheight 8.75in
\oddsidemargin 0.0in
\textwidth 6.5in
...
If the first argument (the program name) ends with the extension {\ex
...
\section{Apache Portable Runtime}

Portability to other operating systems is supported by including the
lua-apr package in the table {\ex apr}.  See
\url{http://peterodding.com/code/lua/apr} for details.  Some of its
commonly used functions are aliased globally to shorten scripts: 

\begin{description}
\item[basename(name{[}, ext{]})] {\ex apr.filepath\_name} --- get last path
element, and optionally return extension separately
\item[pathcat(dir,name)] {\ex apr.filepath\_merge} with the {\ex
native} option set. --- add path elements portably
\item[getcwd()] {\ex apr.filepath\_get} with the {\ex native} option
set. --- return current directory
\item[chdir(dir)] {\ex apr.filepath\_set} --- change working directory
\item[mkdir(dir)] {\ex apr.dir\_make\_recursive} --- create directory and
any missing intervening directories
\item[mv(src, dest)] {\ex apr.file\_rename} --- file rename
\item[cp(src, dest)] {\ex apr.file\_copy} --- file copy
\item[rm(name)] {\ex apr.file\_remove} --- recursive delete
\end{description}
...
apr.env\_get})
\item[exists(name)] --- return true if \emph{name} exists in the file
system.
\item[is\_dir(name)] --- return true if \emph{name} is a directory or a
soft link to a directory.
\item[is\_file(name)] --- return true if \emph{name} is a file or a
soft link to a file.
\item[is\_exec(name)] --- return true if \emph{name} is a file and
...
created as an empty file.  Otherwise, this is the same as {\ex
apr.file\_mtime\_set}.
\end{description}
